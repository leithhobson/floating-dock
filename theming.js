...
var ThemeManager = class DashToDock_ThemeManager {
  ...
  _updateDashOpacity() {
    let newAlpha =
      Docking.DockManager.settings.get_double("background-opacity");
    ...
    let backgroundAlpha = Math.max(
      Math.round(backgroundColor.alpha / 2.55) / 100,
      0.001
    );
  ...
  _getDefaultColors() {
    ...
    if (!this._dash._container.get_stage()) return [null, null];
    ...
    let side = position + 2;
    if (side > 3) side = Math.abs(side - 4);
  ...
  _updateDashColor() {
    ...
    if (settings.get_boolean("custom-background-color")) {
      ...
      if (settings.get_enum("transparency-mode") == TransparencyMode.FIXED) {
        newAlpha = settings.get_double("background-opacity");
        this._customizedBackground = `rgba(${color.red}, ${color.green}, ${color.blue}, ${newAlpha})`;
      } else {
        this._customizedBackground = backgroundColor;
      }
  ...
  _adjustTheme() {
    // Prevent shell crash if the actor is not on the stage.
    // It happens enabling/disabling repeatedly the extension
    if (!this._dash._background.get_stage()) return;
    ...
    let position_keys = ["top", "right", "bottom", "left"];
    ...
    let fixedTransparency =
      settings.get_enum("transparency-mode") == TransparencyMode.FIXED;
    let defaultTransparency =
      settings.get_enum("transparency-mode") == TransparencyMode.DEFAULT;
    ...
    if (!defaultTransparency && !fixedTransparency) {
      ...
      let use_custom_background = settings.get_boolean(
        "custom-background-color"
      );
      let background_color = "";

      if (use_custom_background) {
        const [ret, color] = Clutter.Color.from_string(
      ...
      newStyle =
        newStyle +
        `background-color: ${background_color}; transition-delay: 0s; transition-duration: 0.250s;`;
      this._dash._background.set_style(newStyle);
    }
  }
...
var Transparency = class DashToDock_Transparency {
  ...
  enable() {
    ...
    this._signalsHandler.addWithLabel(
      "transparency",
      [global.window_group, "actor-added", this._onWindowActorAdded.bind(this)],
      [
        global.window_group,
        "actor-removed",
      ...
      [Main.overview, "hiding", this._updateSolidStyle.bind(this)],
      [Main.overview, "showing", this._updateSolidStyle.bind(this)]
    );
    ...
    global.window_group
      ...
      .filter(function (child) {
        ...
        return (
          child instanceof Meta.WindowActor &&
    ...
    if (this._actor.get_stage()) this._updateSolidStyle();
  ...
  _onWindowActorRemoved(container, metaWindowActor) {
    if (!this._trackedWindows.get(metaWindowActor)) return;
  ...
  _dockIsNear() {
    if (this._dockActor.has_style_pseudo_class("overview")) return false;
    /* Get all the windows in the active workspace that are in the primary monitor and visible */
    let activeWorkspace = global.workspace_manager.get_active_workspace();
    let dash = this._dash;
    let windows = activeWorkspace.list_windows().filter(function (metaWindow) {
      return (
        metaWindow.get_monitor() === dash._monitorIndex &&
    ...
    let factor = 0;
    if (
      !Docking.DockManager.settings.dockFixed &&
    ...
    else threshold = topCoord - this._actor.get_height() * factor;
    ...
    let isNearEnough = windows.some((metaWindow) => {
      ...
      if (this._position === St.Side.LEFT) {
        ...
        coord =
          metaWindow.get_frame_rect().x + metaWindow.get_frame_rect().width;
        return coord > threshold - 5 * scale;
      } else if (this._position === St.Side.TOP) {
        ...
        coord =
          metaWindow.get_frame_rect().y + metaWindow.get_frame_rect().height;
        return coord > threshold - 5 * scale;
      }
    });
