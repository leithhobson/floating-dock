...
class MonitorsConfig {
  ...
  static ProxyWrapper = Gio.DBusProxy.makeProxyWrapper(
    MonitorsConfig.XML_INTERFACE
  );
...
var Settings = GObject.registerClass(
  ...
  class DashToDock_Settings extends GObject.Object {
    ...
    custom_opacity_scale_value_changed_cb(scale) {
      ...
      if (this._opacity_timeout > 0) GLib.source_remove(this._opacity_timeout);
    }

    position_top_button_toggled_cb(button) {
      if (button.get_active()) this._settings.set_enum("dock-position", 0);
    }

    position_right_button_toggled_cb(button) {
      if (button.get_active()) this._settings.set_enum("dock-position", 1);
    }

    position_bottom_button_toggled_cb(button) {
      if (button.get_active()) this._settings.set_enum("dock-position", 2);
    }

    position_left_button_toggled_cb(button) {
      if (button.get_active()) this._settings.set_enum("dock-position", 3);
    }

    icon_size_combo_changed_cb(combo) {
      this._settings.set_int(
        "dash-max-icon-size",
        this._allIconSizes[combo.get_active()]
      );
    }
    ...
    all_windows_radio_button_toggled_cb(button) {
      if (button.get_active()) this._settings.set_enum("intellihide-mode", 0);
    }

    focus_application_windows_radio_button_toggled_cb(button) {
      if (button.get_active()) this._settings.set_enum("intellihide-mode", 1);
    }

    maximized_windows_radio_button_toggled_cb(button) {
      if (button.get_active()) this._settings.set_enum("intellihide-mode", 2);
    }
    ...
    always_on_top_radio_button_toggled_cb(button) {
      if (button.get_active()) this._settings.set_enum("intellihide-mode", 3);
    }

    _updateMonitorsSettings() {
      // Monitor options
      const preferredMonitor = this._settings.get_int("preferred-monitor");
      const preferredMonitorByConnector = this._settings.get_string(
        "preferred-monitor-by-connector"
      );
      ...
      for (const monitor of this._monitorsConfig.monitors) {
        if (!monitor.active && monitor.index !== preferredMonitor) continue;
        ...
        if (
          monitor.index === preferredMonitor ||
          (preferredMonitor == -2 &&
    ...
    _bindSettings() {
      ...
      this._builder
        .get_object("intelligent_autohide_button")
        .connect("clicked", () => {
          ...
          let box = this._builder.get_object(
            "intelligent_autohide_advanced_settings_box"
          );
          ...
          intellihideModeRadioButtons[
            this._settings.get_enum("intellihide-mode")
          ].set_active(true);
          ...
          this._settings.bind(
            ...
            this._builder.get_object(
              "autohide_enable_in_fullscreen_checkbutton"
            ),
            "sensitive",
            Gio.SettingsBindFlags.GET
          );
      ...
      const custom_margin_scale = this._builder.get_object(
        "custom_margin_scale"
      );
      ...
      const custom_radius_scale = this._builder.get_object(
        "custom_radius_scale"
      );
      ...
      this._builder
        .get_object("preview_size_scale")
        .set_value(this._settings.get_double("preview-size-scale"));
      ...
      const isolateLocationsBindings = [
        "show_trash_switch",
        "show_mounts_switch",
      ];
      const updateIsolateLocations = () => {
        this._builder.get_object("isolate_locations_row").sensitive =
          isolateLocationsBindings.some(
            (s) => this._builder.get_object(s).active
          );
      };
      updateIsolateLocations();
      isolateLocationsBindings.forEach((s) =>
        this._builder
          .get_object(s)
          .connect("notify::active", () => updateIsolateLocations())
      );
      this._settings.bind(
      ...
      this._builder
        ...
        .set_active(this._settings.get_enum("click-action"));
      this._builder
        .get_object("click_action_combo")
      ...
      this._builder
        .get_object("scroll_action_combo")
        .set_active(this._settings.get_enum("scroll-action"));
      this._builder
        .get_object("scroll_action_combo")
        .connect("changed", (widget) => {
      ...
      this._builder.get_object("overlay_button").connect("clicked", () => {
        ...
        this._builder
          .get_object("overlay_switch")
          .set_active(this._settings.get_boolean("hotkeys-overlay"));
        this._builder
          .get_object("show_dock_switch")
          .set_active(this._settings.get_boolean("hotkeys-show-dock"));
      ...
      this._builder
        ...
        .connect("clicked", () => {
          ...
          this._builder
            .get_object("shift_click_action_combo")
            .set_active(this._settings.get_enum("shift-click-action"));

          this._builder
            .get_object("middle_click_action_combo")
            .set_active(this._settings.get_enum("middle-click-action"));

          this._builder
            .get_object("shift_middle_click_action_combo")
            .set_active(this._settings.get_enum("shift-middle-click-action"));
      ...
      // Running indicators
      this._builder
        .get_object("running_indicators_combo")
        .set_active(this._settings.get_enum("running-indicator-style"));
      this._builder
        .get_object("running_indicators_combo")
        .connect("changed", (widget) => {
          this._settings.set_enum(
      ...
      if (
        ...
        RunningIndicatorStyle.DEFAULT
      )
        this._builder
          .get_object("running_indicators_advance_settings_button")
          .set_sensitive(false);
      ...
      this._builder
        ...
        .connect("clicked", () => {
          ...
          let box = this._builder.get_object(
            "running_dots_advance_settings_box"
          );
          dialog.get_content_area().append(box);
          ...
          rgba.parse(
            this._settings.get_string("custom-theme-running-dots-color")
          );
          this._builder.get_object("dot_color_colorbutton").set_rgba(rgba);

          this._builder
            .get_object("dot_color_colorbutton")
            .connect("notify::rgba", (button) => {
          ...
          rgba.parse(
            this._settings.get_string("custom-theme-running-dots-border-color")
          );
          this._builder
            .get_object("dot_border_color_colorbutton")
            .set_rgba(rgba);
      ...
      this._builder
        .get_object("custom_background_color")
        .connect("notify::rgba", (button) => {
          let css = button.rgba.to_string();
      ...
      // Opacity
      this._builder
        .get_object("customize_opacity_combo")
        .set_active_id(this._settings.get_enum("transparency-mode").toString());
      this._builder
        .get_object("customize_opacity_combo")
        .connect("changed", (widget) => {
          this._settings.set_enum(
            "transparency-mode",
            parseInt(widget.get_active_id())
          );
        });

      this._builder
        .get_object("custom_radius_scale")
        .set_value(this._settings.get_int("border-radius"));

      this._builder
        .get_object("custom_margin_scale")
        .set_value(this._settings.get_int("floating-margin"));

      const custom_opacity_scale = this._builder.get_object(
        "custom_opacity_scale"
      );
      custom_opacity_scale.set_value(
      ...
      this._settings.connect("changed::transparency-mode", () => {
        if (
          ...
          this._builder
            .get_object("dynamic_opacity_button")
            .set_sensitive(false);
        } else {
          this._builder
            .get_object("dynamic_opacity_button")
      ...
      this._builder
        ...
        .connect("clicked", () => {
          ...
          min_alpha_scale.set_value(this._settings.get_double("min-alpha"));
          ...
          max_alpha_scale.set_value(this._settings.get_double("max-alpha"));
      ...
      if (Me)
        this._builder
          ...
          .set_label(Me.metadata.version.toString());
      else this._builder.get_object("extension_version").set_label("Unknown");
    }
  }
);
